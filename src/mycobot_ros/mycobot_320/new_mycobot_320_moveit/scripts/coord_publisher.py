    ...
    def calculate_distances(self):
        ...
        while True:
            ...
            if len(df) > 0:
                ...
                if y1 <= 3:
                    border_message += "Top "
                if x2 >= self.screen_width - 3:
                    border_message += "Right "
                if y2 >= self.screen_height - 3:
                    border_message += "Bottom "
                if border_message != "Border Detected: ":
                    cv2.putText(frame_copy, border_message.strip(), (10, 110), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
            ...
            if center_x != -1 and center_y != -1:
                ...
                # Determine direction
                direction = ""
                if center_x < cam_center_x:
                    direction += f"Left {abs(center_x - cam_center_x)}px, "
                elif center_x >= cam_center_x:
                    direction += f"Right {abs(center_x - cam_center_x)}px, "

                if center_y < cam_center_y:
                    direction += f"Up {abs(center_y - cam_center_y)}px"
                elif center_y >= cam_center_y:
                    direction += f"Down {abs(center_y - cam_center_y)}px"
